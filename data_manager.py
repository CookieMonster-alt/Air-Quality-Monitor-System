...
class DatabaseManager:
    ...
    def execute_ai_read_query(self, query: str) -> Tuple[bool, Union[List[Tuple], str], Union[List[Tuple], None]]:
        """
        Executes a Read-Only SQL query generated by AI.
        Provides a shield against destructive commands.
        Returns (success, result_rows_or_error_message, cursor_description).
        """
        query_upper = query.upper().strip()

        # Security Shield: Block explicitly destructive commands
        forbidden_keywords = ["DROP", "DELETE", "UPDATE", "INSERT", "ALTER", "CREATE", "REPLACE", "TRUNCATE"]
        for keyword in forbidden_keywords:
            # We look for whole words to prevent blocking valid table/column names that might contain them,
            # though in SQLite it's less of an issue. The simplest shield is regex or split, but basic string check works for this scope.
        ...
        try:
            ...
            # For PRAGMA or introspection that might alter things (though rare), this is Read-Only via logic above.
            # However, just to be extremely paranoid, we will rollback any implicit transaction.
            self.conn.rollback()
